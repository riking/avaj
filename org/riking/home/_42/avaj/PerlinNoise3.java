...
/**
 * Perlin noise factory for an arbitrary point in 3 dimensions. The underlying
 * grid is aligned with the integers.
 *
 * There is no limit to the coordinates used; new gradients are generated on
 * the fly as necessary.
 *
 * Implementation based on eevee's code at https://gist.github.com/eevee/26f547457522755cb1fb8739d0ea89a1
 * Licensed under ISC
 */
public class PerlinNoise3 {
    ...
    /**
     * Map from grid point (Integer Vec3) to gradient vector (Unit Vec3)
     */
    ...
    /**
     * Create a new Perlin noise factory in 3 dimensions.
     * <p>
     ...
     * <br>
     * <code>pnf = PerlinNoiseFactory(2, tile=(0, 3))</code>
     * <p>
     * This will produce noise that tiles every 3 units vertically, but never
     * tiles horizontally.
     * <p>
     * If `unbias` is true, the {@link #smoothstep(double)} function will be
     * applied to the output before returning it, to counteract some of Perlin
     * noise's significant bias towards the center of its output range.
     * <p>
     * By default a seed is obtained from {@link java.security.SecureRandom},
     * but you can call {@link #setSeed(long)} to provide your own.
     */
    public PerlinNoise3(int octaves, Vector3 tiling, boolean unbias) {
        // this.dimensions = dimensions;
        if (octaves <= 0) {
            throw new IllegalArgumentException("Number of octaves must be positive");
        }
    ...
    /**
     * Change the seed of the perlin noise object. This resets the object;
     * past results will not be consistent with future calls to get().
     */
    ...
    /**
     * Get plain noise for a single point, without using octaves or tiling.
     */
    public double getPlainNoise(Vector3 point) {
    ...
    // --- End of Perlin functions, remainder of file is Vec3

    /**
     * 3-dimensional vector class. Mutable.
     */
